...
const {generateMessages, generateLocationMessage} = require('./utils/messages.js')
...
io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', (message, callback) => {
        ...
        if(!user) {
            return callback('you are not stored')
        }
    ...
    socket.on('sendLocation', (coords, callback) => {
        ...
        io.to(user.room).emit('locationMessage', generateLocationMessage(user.username, 'https://google.com/maps?q='+coords.lat+','+coords.lang))
        callback()
    })
